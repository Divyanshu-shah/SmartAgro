...
<p align="center">
  <strong>Laravel-based agricultural platform deployed on Render</strong>
</p>
...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/Render-Deployment-46E3B7?style=for-the-badge&logo=render&logoColor=white" alt="Render Deployment" />
</p>
...
> For production on Render, set `APP_URL` to the live Render domain once it is created.
...
This project is deployed as a single Laravel application in a Docker container on Render.
...
4. Use the repo root Dockerfile path: `./Dockerfile`.
5. Add the production environment variables shown above.
6. Deploy the service and update `APP_URL` to the final Render URL.
...
> This app is a Laravel-only project, not a separate frontend/backend split.
...
The app sends automated email notifications through **Resend**:
...
> Ensure the `RESEND_API_KEY` is valid and the sender address is verified in the Resend dashboard for production use.
